...
'---
    ...
    AddRows_TR_FSI 'Uncommented by Shrutika- 19/12/2025 (commented by Chetan C M on 22/10/2025)
    Sheet40.Activate
End Sub
...
Sub settbl_FSI()
    ...
    rngname_FSIfrm = "FSI_CountryCode1;FSI_TaxIdentificationNo1;FSI_IncFromSal1;FSI_TXNOSAL1;FSI_TXNISAL1;FSI_TXRSAL1;FSI_DTAASAL1;FSI_IncFromHP1;FSI_TXNOHP1;FSI_TXNIHP1;FSI_TXRHP1;FSI_DTAAHP1;FSI_IncCapGain1;FSI_TXNOCapGain1;FSI_TXnICapGain1;FSI_TXRCapGain1;FSI_DTAACapGain1;FSI_IncOthSrc1;FSI_TXNOOthSrc1;FSI_TXNIOthSrc1;FSI_TXROthSrc1;FSI_DTAAOthSrc1;FSI_IncT1;FSI_TXNOT1;FSI_TXNIT1;FSI_TXRT1;"
End Sub
...
Function ValidatesheetFSI() As Boolean
    ...
    'start--
    Dim j As Long
    For j = 1 To Sheet40.Range("FSI_Count").value
    If Sheet40.Range("FSI_CountryCode" & j).value = "" Or Sheet40.Range("FSI_CountryCode" & j).value = "(Select)" Then 'Added by Shrutika(18/12/2025)

    If Sheet40.Range("FSI_TaxIdentificationNo" & j).value <> "" Or Sheet40.Range("FSI_IncFromSal" & j).value <> "" Or Sheet40.Range("FSI_TXNOSAL" & j).value <> "" Or Sheet40.Range("FSI_TXNISAL" & j).value <> "" Or Sheet40.Range("FSI_DTAASAL" & j).value <> "" Or Sheet40.Range("FSI_DTAASAL" & j).value <> "" Or Sheet40.Range("FSI_IncFromHP" & j).value <> "" Or Sheet40.Range("FSI_TXNOHP" & j).value <> "" Or Sheet40.Range("FSI_DTAAHP" & j).value <> "" Or Sheet40.Range("FSI_IncCapGain" & j).value <> "" Or Sheet40.Range("FSI_TXNOCapGain" & j).value <> "" Or Sheet40.Range("FSI_TXnICapGain" & j).value <> "" Or Sheet40.Range("FSI_DTAACapGain" & j).value <> "" Or Sheet40.Range("FSI_IncOthSrc" & j).value <> "" Or Sheet40.Range("FSI_TXNOOthSrc" & j).value <> "" Or Sheet40.Range("FSI_TXNIOthSrc" & j).value <> "" Or Sheet40.Range("FSI_DTAAOthSrc" & j).value <> "" Then
        Msgbox_FSI = Msgbox_FSI + "* Country code at Sr. No  " & j & "  in Sheet  FSI  is Mandatory" & Chr(13)
        ValidatesheetFSI = False
    End If
    End If
    Next
    '--end
...
Function ValidateTaxIdentificationNo_FSI() As Boolean
    ...
    rangecells = Sheet40.Range("FSI_TaxIdentificationNo" & i).value
     ...
     'start--
         ...
         Msgbox_FSI = Msgbox_FSI + "* Tax Identification Number is mandatory in Schedule FSI  in Sl.no. " & i & " " & Chr(13)
     '--end
