...
Function MyTotalPTI() As Double 'Updated by Shrutika (10/09/2025 - Tc's)
    'dpk
    Dim te As Long
    MyTotalPTI = 0
...
On Error Resume Next
    ...
    For i = 1 To CntrPTI
        ...
        If NameOfBusiness_PTI_1(i) = "" Then
            msgbox_PTI ("* Name of business trust/ investment fund at Section   " & i & " in Schedule PTI is mandatory.")
            ChkMandPTI = False
            Exit Function
        End If
...
On Error Resume Next
    ...
    For i = 1 To CntrPTI
        ...
        If ((PTI_UsIncomeAmtb_1(i) <> "" Or PTI_UsTDSAmtb_1(i) <> "") And Trim(PTI_Usb_1(i)) = "") Then
            'changed by Chetan C M on 22/09/2025 SIT-89790
        'start--
'            msgbox_PTI ("* U/s(b) under which Income claimed to be exempt at Section  " & i & " in Schedule PTI is mandatory.")
            msgbox_PTI ("* Head of Income is mandatory in Schedule PTI  in Sl.no  " & i & "")
        '--end
            ChkMandPTI_2 = False
            Exit Function
        End If
    ...
    For i = 1 To end_PTI
        ...
        If Not CheckDoneePAN(PANOfBusiness_PTI(i)) Then
           ...
           ' msgbox_PTI ("* PAN Of Business Trust at Section   " & i & " in Schedule PTI is invalid")
            ...
            'changed by Chetan C M on 30/07/2025
            'start--
'            msgbox_PTI ("* ""Invalid PAN in Schedule PTI in Sl.no.1. PAN format should be First 5 Alphabets, next 4 digits, then 1 Alphabet.""")
...
Function ValidateIncFromHP_PTI() As Boolean
    ...
    For i = 1 To end_PTI
        ...
        If Not chkCompulsory(PTI_HPIncomeAmt(i)) Then
            ...
            'Shrutika
            'changed by Chetan C M on 01/08/2025
            'start--
'            msgbox_PTI "* ""Amount of income is mandatory in Schedule PTI  in Sl.no.1"""
              msgbox_PTI "* ""Amount of income is mandatory in Schedule PTI  in Sl.no " & i & """"
            '--end
        ...
        End If
          ...
          If Not chkCompulsory(PTI_HPShareAmt(i)) Then
            'added by Chetan C M on 22/09/2025 SIT-89794
            'start--
              msgbox_PTI ("* Share of current year loss distributed by Investment fund From HP at Section   " & i & "  in Schedule PTI is mandatory. Pl fill zero if no Income.")
               ' msgbox_PTI ("* ""Amount of income is mandatory in Schedule PTI  in Sl.no " & i & """")'Commented by Shrutika(15/10/2025)Confirm by shlipa
            '--end
            ValidateIncFromHP_PTI = False
            Exit Function
        End If
