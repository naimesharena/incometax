...
If Sheet1.Range("sheet1.ReturnFileSec").value = "139(8A)" Then
    ...
    If Sheet201.Range("U_PreviouslyFiledForThisAY").value = "Yes" Then
        If Sheet202.Range("U_LatestTotInc").value = "" Then
           ...
           msgbox_1398AATI = msgbox_1398AATI + """Since ""Yes"" is selected for A5 ""Whether return previously filed for this assessment year?"" in Schedule Part A Gen_139(8A), ""Total income as per last Valid return"" is required to be filled mandatorily.""" & Chr(13)
           Validatesheet_ATI = False
...
Dim vRows As Long
mdCommon.DefinedgridNameRange = ("U_BSRCode1||U_DateDep1||U_SrlNoOfChaln1||U_Amt1||U_DateOfAmount1")
...
Dim vRows As Long
mdCommon.DefinedgridNameRange = ("U_BSRCode2||U_DateDep2||U_SrlNoOfChaln2||U_Amt2")
...
For i = 1 To Sheet202.Range("U_BSRCode1").Rows.count
    If Sheet202.Range("U_BSRCode1").item(i).value <> "" Or Sheet202.Range("U_DateDep1").item(i).value <> "" Or _
        Sheet202.Range("U_SrlNoOfChaln1").Cells(i, 1).value <> "" Or Sheet202.Range("U_Amt1").item(i).value <> "" Then
...
For i = 1 To Sheet202.Range("U_BSRCode2").Rows.count
    If Sheet202.Range("U_BSRCode2").item(i).value <> "" Or Sheet202.Range("U_DateDep2").item(i).value <> "" Or _
        Sheet202.Range("U_SrlNoOfChaln2").Cells(i, 1).value <> "" Or Sheet202.Range("U_Amt2").item(i).value <> "" Then
...
End If
 ...
 If Not Validate_BSRCODE1(BSR_IT2(i)) Then
         msgbox_1398AATI = msgbox_1398AATI + "Invalid BSR code at Sr. No  " & i & " in Schedule IT-2. First three digits from the left stand are for district code; next three digits stand for revenue centre code within the district and last single digit stands for population range code" & Chr(13)
         ValidateBSR_IT2 = False
        Exit Function
     End If
UpdateProgressBar
...
If Not chkCompulsory_1398A(TaxPaid3_IT2(i)) Then
    msgbox_1398AATI = msgbox_1398AATI + "* ""Please enter the Amount at Sr.No " & i & " in Schedule IT-2""" & Chr(13)
