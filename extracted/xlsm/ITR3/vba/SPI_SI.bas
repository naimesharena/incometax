...
Dim endname_SPI, endAmt_SPI, endPan_SPI, endAadhaar_SPI As Long
Dim endRelation_SPI, endIncmNature_SPI As Long

Dim endname_IF, endPan_IF, endisLiable_IF As Long
Dim endProfitShare_IF, endProfitAmt_IF, endCap_IF, endIntrstAmt_IF, endRemunernAmt_IF As Long
...
Public firmIntrstAmt_IF, firmRemunernAmt_IF As Variant
...
EfilingCommon.DefinedgridNameRange = ("IF.FirmName||IF.FirmPAN||IF.IsLiableToAudit||IF.Is92EApplicable||IF.ProfitSharePercent||IF.ProfitShareAmt||IF.IntrstAmtDueOrRecv||IF.RemunernAmtDueOrRecv||IF.FirmCapBalOn31Mar||isLiableForAudit||LiableFor92E")
...
fmsgboxStatus "Sheet SPI-SI-IF is ok"
End Sub
...
'endname_SPI,endRelation_SPI,endIncmNature_SPI,endAmt_SPI,endPan_SPI,endAadhaar_SPI
...
'            MsgSPISheet = MsgSPISheet + "* * Please enter the Name of Person at Sr. No " & i & " in Schedule SPI" & Chr(13)
         'SIT-69222 changed by sai
            MsgSPISheet = MsgSPISheet + "* ""Please enter the Name of Person in schedule SPI""" & Chr(13)
...
'             MsgSPISheet = MsgSPISheet + Chr(13) & "Please enter valid Aadhaar Number in schedule SPI" & Chr(13)
             ...
             'SIT-69222 changed by sai
             MsgSPISheet = MsgSPISheet + Chr(13) & """Invalid Aadhaar in Schedule SPI """ & Chr(13)
...
setTblinfoIntrstAmt_IF
setTblinfoRemunernAmt_IF
...
'End If
            ...
            If Not ValidateIntrstAmt_IF Then ValidateSheetIF = False
            If Not ValidateRemunernAmt_IF Then ValidateSheetIF = False
            
            If Not ValidateFirmCapital_IF Then ValidateSheetIF = False
            If Not validateTotalIF Then ValidateSheetIF = False
 'End If
For i = 1 To Sheet21.Range("IF.FirmName").count

    If Sheet21.Range("IF.FirmName").Cells(i, 1).value <> "" Or Sheet21.Range("IF.FirmPAN").Cells(i, 1).value <> "" Or Sheet21.Range("IF.IsLiableToAudit").Cells(i, 1).value <> "" And Sheet21.Range("IF.IsLiableToAudit").Cells(i, 1).value <> "(Select)" Or _
        (Sheet21.Range("IF.Is92EApplicable").Cells(i, 1).value <> "" And Sheet21.Range("IF.Is92EApplicable").Cells(i, 1).value <> "(Select)" Or Sheet21.Range("IF.ProfitSharePercent").Cells(i, 1).value <> "" And Sheet21.Range("IF.ProfitShareAmt").Cells(i, 1).value <> "" Or Sheet21.Range("IF.IntrstAmtDueOrRecv").Cells(i, 1).value <> "" Or Sheet21.Range("IF.RemunernAmtDueOrRecv").Cells(i, 1).value <> "") Or _
        Sheet21.Range("IF.FirmCapBalOn31Mar").Cells(i, 1).value <> "" Then
   
        
        If Sheet21.Range("IF.FirmName").Cells(i, 1).value = "" Then
            ValidateSheetIF = False
            MsgIFSheet = MsgIFSheet + "* Please enter the Name of the Firm in schedule IF at Sr. No " & i & "" & Chr(13)
        End If
        
        If Sheet21.Range("IF.FirmPAN").Cells(i, 1).value = "" Then
            ValidateSheetIF = False
            MsgIFSheet = MsgIFSheet + "* Please enter PAN of the Firm in schedule IF at Sr. No " & i & "." & Chr(13)
        End If

       If Sheet21.Range("IF.IsLiableToAudit").Cells(i, 1).value = "" Or Sheet21.Range("IF.IsLiableToAudit").Cells(i, 1).value = "(Select)" Then
            ValidateSheetIF = False
            MsgIFSheet = MsgIFSheet + "*Please select an Option from Drop-Down in schedule IF at Sr. No " & i & "." & Chr(13)
        End If

        If Sheet21.Range("IF.Is92EApplicable").Cells(i, 1).value = "" Or Sheet21.Range("IF.Is92EApplicable").Cells(i, 1).value = "(Select)" Then
            ValidateSheetIF = False
            MsgIFSheet = MsgIFSheet + "* Please select an Option from Drop-Down in schedule IF at Sr. No " & i & "." & Chr(13)
        End If


        If Sheet21.Range("IF.ProfitSharePercent").Cells(i, 1).value = "" Then
            ValidateSheetIF = False
            MsgIFSheet = MsgIFSheet + "*Please enter the Percentage share in the profit of the firm in schedule IF at Sr. No " & i & "" & Chr(13)
        End If

        If Sheet21.Range("IF.ProfitShareAmt").Cells(i, 1).value = "" Then
            ValidateSheetIF = False
            MsgIFSheet = MsgIFSheet + "* Please enter the amount of share in Profit in schedule IF at Sr. No " & i & "" & Chr(13)
        End If
        
          If Sheet21.Range("IF.IntrstAmtDueOrRecv").Cells(i, 1).value = "" Then
            ValidateSheetIF = False
            MsgIFSheet = MsgIFSheet + "* Please provide the amount of interest due or received in Schedule IF at Sr. No " & i & "" & Chr(13)
        End If
        
          If Sheet21.Range("IF.RemunernAmtDueOrRecv").Cells(i, 1).value = "" Then
            ValidateSheetIF = False
            MsgIFSheet = MsgIFSheet + "* Please provide the amount of remuneration due or received in Schedule IF at Sr. No " & i & "" & Chr(13)
        End If

        If Sheet21.Range("IF.FirmCapBalOn31Mar").Cells(i, 1).value = "" Then
            ValidateSheetIF = False
            MsgIFSheet = MsgIFSheet + "* Please enter the capital Balance as on 31st March in the firm in schedule IF at Sr. No " & i & "" & Chr(13)
        End If
...
'          'SIT-69222 changed by sai
'            MsgIFSheet = MsgIFSheet + "* Please select an Option from Drop-Down in schedule IF" & Chr(13)
'
'
...
'        'SIT-69222 changed by sai
'            MsgIFSheet = MsgIFSheet + "* ""Please enter the Percentage share in the profit of the firm in schedule IF""" & Chr(13)
'
'
...
'          'SIT-69222 changed by sai
'            MsgIFSheet = MsgIFSheet + "* ""Please enter the amount of share in Profit in schedule IF""" & Chr(13)
'
'
...
'        End If
    Next
End Function
Function ValidateIntrstAmt_IF() As Boolean

ValidateIntrstAmt_IF = True
    setTblinfoName_IF
    Dim rangecells As Range
    Set rangecells = Sheet21.Range("IF.IntrstAmtDueOrRecv").Cells
    Dim i As Long
    ReDim firmIntrstAmt_IF(endname_IF)
    For i = 1 To endname_IF
        firmIntrstAmt_IF(i) = rangecells.item(i).value
        
    Next
End Function
Function ValidateRemunernAmt_IF() As Boolean

ValidateRemunernAmt_IF = True
    setTblinfoName_IF
    Dim rangecells As Range
    Set rangecells = Sheet21.Range("IF.RemunernAmtDueOrRecv").Cells
    Dim i As Long
    ReDim firmRemunernAmt_IF(endname_IF)
    For i = 1 To endname_IF
        firmRemunernAmt_IF(i) = rangecells.item(i).value
...
'
'            MsgIFSheet = MsgIFSheet + "* ""Please enter the capital Balance as on 31st March in the firm in schedule IF""" & Chr(13)
'
'
...
Dim total1, total2, total3, total4 As Variant
    total1 = Sheet21.Range("IF.TotalProfitShareAmt").value
    total2 = Sheet21.Range("IF.TotalFirmCapBalOn31Mar").value
    
    total3 = Sheet21.Range("IF.TotalIntrstAmtDueOrRecv").value
    total4 = Sheet21.Range("IF.TotalRemunernAmtDueOrRecv").value
    ...
    End If
      ...
      If Len(total3) > 14 Then
    MsgIFSheet = MsgIFSheet + "* Total of interest Amount due or received cannot be greater than 14 digits. in Schedule IF" + Chr(13)
    validateTotalIF = False
    End If
    
      If Len(total4) > 14 Then
    MsgIFSheet = MsgIFSheet + "* Total of remuneration Amount due or received cannot be greater than 14 digits. in Schedule IF" + Chr(13)
    validateTotalIF = False
    End If
...
End Sub
Sub setTblinfoIntrstAmt_IF()
 Dim rangecells As Range
 Dim mIntCells As Long
 Dim mIntCtr As Long
 Dim ccount As Long
 ccount = 0
 mIntCells = Sheet21.Range("IF.IntrstAmtDueOrRecv").count
 Set rangecells = Sheet21.Range("IF.IntrstAmtDueOrRecv").Cells
 For mIntCtr = 1 To mIntCells
     If rangecells.item(mIntCtr).value <> "" Then
         ccount = ccount + 1
     End If
 Next
 endIntrstAmt_IF = ccount
End Sub
Sub setTblinfoRemunernAmt_IF()
 Dim rangecells As Range
 Dim mIntCells As Long
 Dim mIntCtr As Long
 Dim ccount As Long
 ccount = 0
 mIntCells = Sheet21.Range("IF.RemunernAmtDueOrRecv").count
 Set rangecells = Sheet21.Range("IF.RemunernAmtDueOrRecv").Cells
 For mIntCtr = 1 To mIntCells
     If rangecells.item(mIntCtr).value <> "" Then
         ccount = ccount + 1
