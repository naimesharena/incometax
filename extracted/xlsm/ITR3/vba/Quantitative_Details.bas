...
'Change-20.04.2023.101.QD -> SIT-48846
    Dim cell
    Dim row As Long
    row = 0
    For Each cell In Sheet6.Range("QDTradingConcern.ItemName").Cells
        row = row + 1
        If (Sheet6.Range("QDTradingConcern.ItemName").item(row, 1).value <> "" Or (Not isdropdownblank(Sheet6.Range("QDTradingConcern.UnitOfMeasure").item(row, 1).value)) Or _
        Sheet6.Range("QDTradingConcern.OpeningStock").item(row, 1).value <> "" Or Sheet6.Range("QDTradingConcern.PurchaseQty").item(row, 1).value <> "" Or _
        Sheet6.Range("QDTradingConcern.SaleQty").item(row, 1).value <> "" Or Sheet6.Range("QDTradingConcern.ClgStock").item(row, 1).value <> "" Or _
        Sheet6.Range("QDTradingConcern.AnyShortExces").item(row, 1).value <> "") Then
            
            If Sheet6.Range("QDTradingConcern.ItemName").item(row, 1).value = "" Then
                msgbox_QDTradingConcern ("* Please enter Item name  at Sr. No  " & i & " in schedule Quantitative Details")
                ValidateItemName_QDTradingConcern = False
                Exit Function
            End If
...
''Change,17.04.2023.102.SIT 48846
'        If Not chkCompulsory(ItemName_QDRawMaterial(i)) Then
'            msgbox_QDRawMaterial ("* Please enter Item name in schedule Quantitative Details at Sr. No  " & i & "")
'            ValidateItemName_QDRawMaterial = False
'            Exit Function
'        End If
''End Change SIT 48846
...
'Change-20.04.2023.101.QD -> SIT-48846
    ...
    Dim row As Long
    row = 0
    For Each cell In Sheet6.Range("QDRawMaterial.ItemName").Cells
        row = row + 1
        If (Sheet6.Range("QDRawMaterial.ItemName").item(row, 1).value <> "" Or (Not isdropdownblank(Sheet6.Range("QDRawMaterial.UnitOfMeasure").item(row, 1).value)) Or _
        Sheet6.Range("QDRawMaterial.OpeningStock").item(row, 1).value <> "" Or Sheet6.Range("QDRawMaterial.PurchaseQty").item(row, 1).value <> "" Or _
        Sheet6.Range("QDRawMaterial.SaleQty").item(row, 1).value <> "" Or Sheet6.Range("QDRawMaterial.ClgStock").item(row, 1).value <> "" Or _
        Sheet6.Range("QDRawMaterial.AnyShortExces").item(row, 1).value <> "") Then
            
            If Sheet6.Range("QDRawMaterial.ItemName").item(row, 1).value = "" Then
                msgbox_QDRawMaterial ("* Please enter Item name  at Sr. No  " & i & " in schedule Quantitative Details")
                ValidateItemName_QDRawMaterial = False
                Exit Function
            End If
        End If
