...
ValidateNOBGrid = True
    ...
    If Not ValidateSubCode_NOB Then ValidateNOBGrid = False

End Function
Function ValidateSubCode_NOB() As Boolean
ValidateSubCode_NOB = True
Dim i As Long
For i = 1 To Sheet28.Range("TCS1.CreditName").count
    If (Sheet2.Range("NOB.TradeNameOrDescription").Cells(i, 1).value <> "" Or Sheet2.Range("NOB.TradeNameOrDescription2").Cells(i, 1).value <> "") Then
        If Sheet2.Range("NOB.Code").item(i).value = "" Or Sheet2.Range("NOB.Code").item(i).value = Empty Or Sheet2.Range("NOB.Code").item(i).value = "(Select)" Then
            MsgBx_NOB = MsgBx_NOB + "* Please select  Nature of Business from the drop down in schedule Nature of Business at Sr. No. :" & i
            ValidateSubCode_NOB = False
            Exit Function
        End If
    End If
